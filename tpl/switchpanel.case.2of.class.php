	...
	class switchpanel_case_2of{
		...
		$sSvg = '';
			...
			$sSvg .= '<rect x="'.$iX.'" y="'.$iY.'" width="'.$opt[ 'elementWidth' ].'" height="'.$opt[ 'elementHeight' ].'" fill="'.$sColor.'"/>
				...
				<circle cx="'.( $iX + ( $opt[ 'elementWidth' ] / 3.2 ) ).'" cy="'.( $iY + ( $opt[ 'elementHeight' ] / 1.4 ) ).'" r="5" fill="#8C7A0F" />
				<circle cx="'.( $iX + ( $opt[ 'elementWidth' ] / 1.45 ) ).'" cy="'.( $iY + ( $opt[ 'elementHeight' ] / 1.4 ) ).'" r="5" fill="#0F8C12" />';
